...
class MQTTClient():
    ...
    def connect(self):
        ...
        try:
            ...
            self.__client.loop_start()  # Starts a single network thread
    ...
    def disconnect(self):
        ...
        if self.__client:
            self.__client.loop_stop()  # Stop the network loop thread first
    ...
    def __on_disconnect(self, client, userdata, rc):
        ...
        # If a reconnect attempt (explicit or background) succeeds, __on_connect sets it back.
        self.__status = MQTTStatus.Disconnected
        if rc != 0:
            print(f"[MQTT] Unexpected disconnection. Error code: {rc}. "
                  "Background reconnection will continue while the loop is active.")
            # The client's loop (started by loop_start()) automatically attempts to reconnect.
            # Do NOT call reconnect() here: it is blocking and races the loop thread.
        else:
